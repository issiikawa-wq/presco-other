...
def get_conversion_name(site_name):
    """サイト名からConversion Nameを取得"""
    conversion_map = {
        "Fast Baito 介護特化": "介護基本",
        "Fast Baito": "FB基本"
    }
    return conversion_map.get(site_name, "その他")
...
def transform_csv_data(csv_path, existing_gclids):
    ...
    transformed_data = []
    # 対象サイト: 介護特化とFast Baito
    target_site_names = ["Fast Baito 介護特化", "Fast Baito"]
    ...
    for row in data_rows:
        if len(row) < 18:  # R列（インデックス17）まで必要
            continue
        ...
        # 対象サイトかチェック
        if site_name not in target_site_names:
            filtered_count += 1
            continue
        ...
        # R列（インデックス17）: 成果報酬単価
        reward_price = row[17] if len(row) > 17 else "0"
        
        # カンマを削除して数値に変換
        try:
            conversion_value = reward_price.replace(',', '').strip()
            if not conversion_value:
                conversion_value = "0"
        except:
            conversion_value = "0"
        
        # サイト名に応じてConversion Nameを設定
        ...
        # サイト別カウント
        if site_name not in site_counts:
